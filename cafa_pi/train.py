...
SEQUENCE_PATH = './data/CAFA_PI_Swissprot_sequence.tsv.gz'
ann_path = './data/CAFA_PI_Swissprot_propagated.tsv.gz'

ann_limit = 5000 # Taking top N GO annotations only
timesteps = 1500 # maximum length of a sequence, the real max is 35K. 2.5K covers 99% of the sequences, 5K 99.9%
latent_dim = 50 # Amino acid embedding size
batch_size = 500 # Warning: this is actually the number of batches in the new Keras API
...
model_dir = './develtestfeaturestwoclasses_model/' # path for saving model + other required stuff
...
def generate_data(split_path, seq_path, ann_path, ann_ids, batches=125, cafa_targets=False, verbose=False, endless=True):
    ...
    while True:
        for i, batch in enumerate(np.array_split(list(pairwise(seq_data)), batches)):
            if verbose:
                print i, len(batch)
            prot_ids = []
            x = []
...
class Evaluate(keras.callbacks.Callback):
    def __init__(self, data_path, steps, reverse_ann_ids):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        gold = np.concatenate([self.data.next()[0]['labels'] for i in range(self.steps)]) # If we consume a full cycle of the generator, we should have gold labels aligned with the predictions
...
def train():
    ...
    # import pdb; pdb.set_trace()
    devel_path = './data/develtest.txt.gz'
    ...
    for i in [3, 9, 27, 81]:
        encoded = Convolution1D(200, i, padding='valid', activation='relu')(embedding)
        encoded = GlobalMaxPooling1D()(encoded)
    ...
    ev_cb = Evaluate(devel_path, batch_size, reverse_ann_ids)
...
def weighted_binary_crossentropy(target, output):
    ...
    pos_weight = 100.0#3.0 # 73 is roughly the inverse ratio of positives examples
